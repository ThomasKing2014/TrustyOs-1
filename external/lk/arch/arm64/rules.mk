...
SMP_CPU_ID_BITS ?= 12 # Ignore aff3 bits for now since they are not next to aff2
...
$(BUILDDIR)/system-onesegment.ld: $(LOCAL_DIR)/system-onesegment.ld $(wildcard arch/*.ld)
	@echo generating $@
	@$(MKDIR)
	$(NOECHO)sed "s/%MEMBASE%/$(MEMBASE)/; \
	  s/%MEMSIZE%/$(MEMSIZE)/; \
	  s/%KERNEL_BASE%/$(KERNEL_BASE)/; \
	  s/%KERNEL_LOAD_OFFSET%/$(KERNEL_LOAD_OFFSET)/; \
	  s/%BOOT_MEM_OFFSET%/$(BOOT_MEM_OFFSET)/ \
	  " < $< > $@
