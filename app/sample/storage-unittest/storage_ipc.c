...
static unsigned char chan_msg_buf[STORAGE_UNITTEST_MAX_BUFFER_SIZE];


extern int handle_client_msg (struct evt_context *ctx, void *msg_buf, size_t msg_size);

static int handle_chan_msg(struct evt_context *chan_ctx, const uevent_t *ev)
{
	...
	if (msg_inf.len > STORAGE_UNITTEST_MAX_BUFFER_SIZE) {
	...
	rc = chan_ctx->msg_handler(chan_ctx, chan_msg_buf, msg_inf.len);
...
static void chan_evt_handle(const uevent_t *ev)
{
    struct evt_context *chan_ctx;
    int rc;
    ...
    if (ev->event & (IPC_HANDLE_POLL_MSG |
		...
		if (NULL != chan_ctx->msg_handler) {
			rc = handle_chan_msg(chan_ctx, ev);
			if (rc < 0) {
...
void port_evt_handle(const uevent_t *ev)
{
	...
	if (ev->event & IPC_HANDLE_POLL_READY) {
		...
		chan_ctx = calloc(1, sizeof(struct evt_context));
		if (!chan_ctx) {
		...
		/* init state */
		chan_ctx->evt_handler = chan_evt_handle;
        chan_ctx->msg_handler = handle_client_msg;
...
void dispatch_evt(const uevent_t *ev)
{
	...
	struct evt_context *ctx = ev->cookie;
