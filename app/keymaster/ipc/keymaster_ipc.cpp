...
uuid_t fpsensor_uuid = {
         0x4304bef6, 0x36e5, 0x4d91, { 0x94, 0xb0, 0x1e, 0xa4, 0xcd, 0x51, 0xd4, 0x0c }};
...
static long keymaster_dispatch_non_secure(keymaster_chan_ctx* ctx, keymaster_message* msg,
    ...
    case KM_ATTEST_KEY:
        LOG_D("Dispatching KM_ATTEST_KEY, size: %d", payload_size);
        return do_dispatch(&TrustyKeymaster::AttestKey, msg, payload_size, out,out_size);

    case KM_UPGRADE_KEY:
        LOG_D("Dispatching KM_UPGRADE_KEY, size: %d", payload_size);
        return do_dispatch(&TrustyKeymaster::UpgradeKey, msg, payload_size, out,
                           out_size);

    case KM_CONFIGURE:
        LOG_D("Dispatching KM_CONFIGURE_OPERATION, size %d", payload_size);
        return do_dispatch(&TrustyKeymaster::ConfigureOperation, msg, payload_size, out, out_size);
...
static bool keymaster_port_accessible(uuid_t* uuid, bool secure) {
    //return !secure || (memcmp(uuid, &gatekeeper_uuid, sizeof(gatekeeper_uuid)) == 0) || (memcmp(uuid, &fpsensor_uuid, sizeof(fpsensor_uuid)) == 0);
    return true;
}
