                      ...
                      iovec_t *rx_iovecs, size_t rx_iovec_count)
{
	...
	if (rc == ERR_NOT_ENOUGH_BUFFER) {
		TLOGE("%s: wxw buffer not enough,wait to send_msg\n", __func__);
	...
	printf("%s wxw: handle 0x%x send_msg complete in %lld ns\n", __func__, session, time_start_ns);
#endif
	...
	printf("%s wxw: in %lld ns, wait handle 0x%x 's response use %lld ns\n", __func__,
			time_end_ns, session, time_end_ns - time_start_ns);
#endif
...
#if PRINT_TIME
        gettime(0, 0, &time_start_ns);
        TLOGE("wxw ---- %s: event 0x%x handle 0x%x in %lld ns\n",
				__func__, event.event, event.handle, time_start_ns);
#endif
...
#if PRINT_TIME
        gettime(0, 0, &time_end_ns);
        TLOGE("wxw ---- %s: use %lld ns\n", __func__, time_end_ns - time_start_ns);
#endif
	}
}
