...
{
    const struct block_tree_node *node_ro;
    const struct block_tree_node *parent_node_ro = NULL;
    obj_ref_t ref[2] = {
        OBJ_REF_INITIAL_VALUE(ref[0]),
        OBJ_REF_INITIAL_VALUE(ref[1]),
    };
    ...
    const struct block_mac *next_child = NULL ;
                                                     ...
                                                     struct block_tree_path *path)
{
    ...
    assert(path->tree);
    assert(path->count > 1);
    memset(block_mac.data , 0 , sizeof(block_mac.data)) ; // wqc add 2017-07-18
