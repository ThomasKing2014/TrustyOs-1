...
/**
 * check_storeage_sync - Check storeage whether synchronization
 * @tr:         Transaction object.
 *
 */
static int check_storeage_sync(struct transaction *tr)
{
    int ret;

    ret = tr->fs->dev->sync(tr->fs->dev);
...
void transaction_complete(struct transaction *tr)
{
    ...
    if (check_storeage_sync(tr) != 0) {
        tr->failed = true;
        pr_err("transaction failed, check_storeage_sync() fialed.\n");
        goto err_transaction_failed;
    }
