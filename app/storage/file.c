...
#if PRINT_TIME
    pr_err("block_map %lld\n", block_mac_to_block(tr, &file_entry_ro->block_map));
#endif
 ...
 *
 */
void file_block_map_update(struct transaction *tr,
                                  struct block_map *block_map,
                                  struct file_handle *file)
{
    bool found;
...
/**
 * file_get_block_new - Helper function to get a file block for read or write
 * @tr:              Transaction object.
 * @block_map:       file's Block map object.
 * @file_block_mac:  @return block data 's block_mac
 ...
 */
void *file_get_block_new(struct transaction *tr,
                                      struct block_map *block_map,
                                      struct block_mac  *file_block_mac,
                                      data_block_t file_block,
                                      bool read,
                                      bool write,
                                      obj_ref_t *ref)
{
    ...
    found = block_map_get(tr, block_map, file_block, file_block_mac);
    if (found) {
        if (read) {
            data = block_get(tr, file_block_mac, NULL, ref); /* TODO: pass iv? */
        } else {
            data = block_get_no_read(tr, block_mac_to_block(tr, file_block_mac), ref);
        }
        /* TODO: handle mac mismatch better */
    }

    old_disk_block = found ? block_mac_to_block(tr, file_block_mac) : 0;
#if PRINT_TIME
    pr_err("file_data old %lld\n",old_disk_block);
#endif
    if (write && (!found || transaction_block_need_copy(tr, old_disk_block))) {
        ...
        assert(new_block);
        block_mac_set_block(tr, file_block_mac, new_block);

        if (found) {
#if PRINT_TIME
            pr_err("file_data move %lld, to %lld\n", old_disk_block, new_block);
#endif
    ...
    return (void *)(data + sizeof(struct iv));
...
#if PRINT_TIME
    pr_err("tr->files_added %lld\n", block_mac_to_block(tr,&(tr->files_added.root)));
#endif
    block_tree_insert_block_mac(tr, &tr->files_added, hash, *block_mac_out);
#if PRINT_TIME
    pr_err("tr->files_added %lld\n", block_mac_to_block(tr,&(tr->files_added.root)));
    pr_err("tr->freed %lld\n", block_mac_to_block(tr,&(tr->freed.block_tree.root)));
    pr_err("tr->tmp_allocated %lld\n", block_mac_to_block(tr,&(tr->tmp_allocated.block_tree.root)));
    pr_err("tr->allocated %lld\n", block_mac_to_block(tr,&(tr->allocated.block_tree.root)));
#endif
    if (tr->failed) {
        pr_warn("transaction failed, abort\n");
        goto err;
...
/**
 * file_delete - rename file
 * @tr:         Transaction object.
 * @old_path:   Path to rename file at.
 * @old_path:   file's new name.
 *
 * Return: %true if file was found, %false otherwise.
 */
bool file_rename(struct transaction *tr, const char *old_path, const char *new_path)
{
    bool found;
    struct block_mac block_mac;
    data_block_t new_block;
    const struct file_entry *file_entry_ro = NULL;
    struct file_entry *file_entry_rw = NULL;
    obj_ref_t file_entry_old_ref = OBJ_REF_INITIAL_VALUE(file_entry_ref);
    obj_ref_t file_entry_copy_ref = OBJ_REF_INITIAL_VALUE(file_entry_ref);
    obj_ref_t *file_entry_ref = &file_entry_old_ref;
    ...
    //Is new file's name valid?
    found = file_tree_lookup(&block_mac, tr, &tr->fs->files, &tree_path,
                             new_path, false);
    if (found) {
        pr_err("the new file %s alread exist\n", new_path);
        return false;
    }
    ...
    found = file_tree_lookup(&block_mac, tr, &tr->files_added, &tree_path,
                             new_path, false);
    if (found) {
        pr_err("the new file %s alread in tr->files_added\n", new_path);
        return false;
    }

    //Is old file's name valid?
    found = file_tree_lookup(&block_mac, tr, &tr->fs->files, &tree_path,
                             old_path, false);
    if (!found) {
    ...
    found = file_tree_lookup(&block_mac, tr, &tr->files_added, &tree_path,
                             old_path, false);
    if (found) {
        pr_err("file %s in tr->files_added\n", old_path);
        return false;
    }

    if (file_is_removed(tr, block_mac_to_block(tr, &block_mac))) {
        pr_err("file %s is re removed\n", old_path);
        return false;
    }
    ...
    if (block_tree_path_get_key(&tree_path) == block_mac_to_block(tr, &block_mac)) {
       pr_err("file %s, %lld, already updated in this transaction",
               old_path, block_mac_to_block(tr, &block_mac));
       return false;
    }

    open_file = file_find_open(tr, &block_mac);
    if (open_file) {
        pr_err("file %s alread open\n", old_path);
        return false;
    }


    pr_write("rename file, %s --> %s, at block %lld\n",
             old_path, new_path, block_mac_to_block(tr, &block_mac));
    ...
    assert(file_entry_ro);
    assert(!strcmp(file_entry_ro->path, old_path));
    ...
    assert(new_block);
    file_entry_rw = block_get_copy(tr, file_entry_ro, new_block,
								   false, &file_entry_copy_ref); // TODO:

    strcpy(file_entry_rw->path, new_path);
    hash = path_hash(tr, new_path);
    block_mac_set_block(tr, &block_mac_out, new_block);
    block_put_dirty(tr, file_entry_rw, &file_entry_copy_ref, &block_mac_out, NULL);
    block_tree_insert_block_mac(tr, &tr->files_added, hash, block_mac_out);
