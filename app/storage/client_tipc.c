...
static enum storage_err storage_file_write_new(struct storage_msg *msg,
                    ...
                    struct storage_client_session *session)
{
...
#if PRINT_TIME
	int64_t time_start_ns = 0,time_end_ns = 0;
	gettime(0, 0, &time_start_ns);
#endif
	...
	while (bytes_left) {
		...
		SS_ERR("w %lld\n", block_num);
#endif
		block_data = (void *)file_get_block_new(tr, &block_map,
                                    &file_block_mac, block_num,
		                            len != block_size, true, &block_data_ref);
		if (!block_data) {
		...
		block_map_set(tr, &block_map, block_num, &file_block_mac);

		block_map_put_dirty(tr, &block_map, block_num,
                        block_data - sizeof(struct iv), &block_data_ref);
...
#if PRINT_TIME
	gettime(0, 0, &time_end_ns);
	SS_ERR("wn %lld,use %lld ns\n", block_num, time_end_ns - time_start_ns);
#endif
...
static enum storage_err storage_file_rename(struct storage_msg *msg,
                        struct storage_file_rename_req *req, size_t req_size,
                        struct storage_client_session *session)
{
	const char *old_fname, *new_fname;
	size_t old_fname_len,new_fname_len;
	char old_path_buf[FS_PATH_MAX],new_path_buf[FS_PATH_MAX];
	enum storage_err result;
	bool renamed;
	...
	/* make sure filename is legal */
	old_fname = req->old_file_name;
	old_fname_len = strlen(req->old_file_name);
	if (!is_valid_name(old_fname, old_fname_len)) {
		SS_ERR("%s: invalid filename\n", __func__);
		return STORAGE_ERR_NOT_VALID;
	}

	result = get_path(old_path_buf, sizeof(old_path_buf), &session->uuid,
                      old_fname, old_fname_len);
	if (result != STORAGE_NO_ERROR) {
		SS_ERR("%s: get path of file:%s error\n", __func__, old_fname);
		return STORAGE_ERR_NOT_VALID;

	}

	new_fname = req->new_file_name;
	new_fname_len = strlen(req->new_file_name);
	if (!is_valid_name(new_fname, new_fname_len)) {
		SS_ERR("%s: invalid filename\n", __func__);
		return STORAGE_ERR_NOT_VALID;
	}

	result = get_path(new_path_buf, sizeof(new_path_buf), &session->uuid,
                      new_fname, new_fname_len);
	if (result != STORAGE_NO_ERROR) {
		SS_ERR("%s: get path of file:%s error\n", __func__, new_fname);
		return STORAGE_ERR_NOT_VALID;
	...
	SS_INFO("%s: rename %s to %s \n", __func__, old_path_buf, new_path_buf);

	renamed = file_rename(&session->tr, old_path_buf, new_path_buf);

	if (session->tr.failed) {
		SS_ERR("%s: transaction failed\n", __func__);
		return STORAGE_ERR_GENERIC;
	} else if (!renamed) {
		SS_ERR("%s: rename failed\n", __func__);
		return STORAGE_ERR_NOT_FOUND;
	}
...
static int client_handle_msg(struct ipc_channel_context *ctx, void *msg_buf, size_t msg_size)
{
	...
	case STORAGE_FILE_WRITE:
		result = storage_file_write_new(msg, payload, payload_len, session);
		break;
	...
	case STORAGE_FILE_SET_SIZE:
		...
		break;
	case STORAGE_FILE_RENAME:
		result = storage_file_rename(msg, payload, payload_len, session);
		break;
	default:
