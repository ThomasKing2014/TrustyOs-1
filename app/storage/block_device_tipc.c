...
#define SS_INFO(args...)  fprintf(stdout, "ss: " args)
//#define SS_DBG_IO(args...)  fprintf(stderr, "ss: " args)
...
static int block_device_tipc_rpmb_sync(struct block_device *dev)
{
    return 0;
}
                                             ...
                                             size_t data_size)
{
    ...
    SS_ERR("TN %lld, use %lld ns\n", block, time_end_ns - time_start_ns);
#endif
    SS_DBG_IO("%s: block %lld, ret %d\n", __func__, block, ret);
...
static int block_device_tipc_ns__sync(struct block_device *dev)
{
    int ret;
    struct block_device_tipc *state = dev_ns_to_state(dev);

    ret = storage_sync_check(state->ipc_handle);
                                            ...
                                            uint32_t block_count)
{
    ...
    dev_rpmb->dev.sync = block_device_tipc_rpmb_sync;
...
int block_device_tipc_rpmb_init(struct block_device_tipc *state,
				handle_t ipc_handle,
				const struct key *fs_key)
{
	...
	if (BLOCK_COUNT_RPMB) {
		...
		/* TODO: get hint from ns */
		rpmb_block_count = rpmb_search_size(state, 0);
	...
	SS_INFO("%s: rpmb device's block size %d block count %lld\n",
			__func__, state->dev_rpmb.dev.block_size, state->dev_rpmb.dev.block_count);
	...
	if (ret < 0) {
		SS_ERR("%s: %s 's fs init failed\n", __func__, STORAGE_CLIENT_TP_PORT);
	...
	if (ret < 0) {
		SS_ERR("%s: create port %s failed\n", __func__, STORAGE_CLIENT_TP_PORT);
		goto err_fs_rpmb_create_port;
	}
	SS_INFO("%s: create port %s success\n", __func__, STORAGE_CLIENT_TP_PORT);
	...
	if (ret < 0) {
		SS_ERR("%s: create port %s failed\n", __func__, STORAGE_CLIENT_TDEA_PORT);
...
err_bad_rpmb_size:
	state->dev_rpmb.dev.block_count = 0;

	return ret;
}

int block_device_tipc_ns_init(struct block_device_tipc *state,
				handle_t ipc_handle,
				const struct key *fs_key)
{
	int ret;
	...
	state->dev_ns.sync = block_device_tipc_ns__sync;
	...
	SS_INFO("%s: ns device's block size %d block count %lld\n",
			__func__, state->dev_ns.block_size, state->dev_ns.block_count);

	ret = ns_open_file(state->ipc_handle, "0", &state->ns_handle, true);
	if (ret < 0) {
		SS_ERR("%s: open '0' failed, port %s don't use, RPMB fs only.\n",
				__func__, STORAGE_CLIENT_TD_PORT);
		/* RPMB fs only */
	...
	state->dev_ns.tamper_detecting = true;

	ret = fs_init(&state->tr_state_ns, fs_key,
		&state->dev_ns, &state->dev_ns, new_ns_fs);
	if (ret < 0) {
		SS_ERR("%s: %s 's fs init failed\n", __func__, STORAGE_CLIENT_TD_PORT);
...
{
	SS_ERR("%s: dev_ns.block_count %lld, dev_rpmb.dev.block_count %lld\n", __func__,
			state->dev_ns.block_count, state->dev_rpmb.dev.block_count);
	...
	if (state->dev_rpmb.dev.block_count) {
