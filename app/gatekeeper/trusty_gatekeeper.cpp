...
bool TrustyGateKeeper::ReseedRng() {
    ...
    if (trusty_rng_secure_rand(rand_seed.get(), RNG_RESEED_SIZE) != NO_ERROR) {
        TLOGE("trusty_rng_secure_rand return false\n");
        return false;
    }
...
long TrustyGateKeeper::DeriveMasterKey() {
    uint8_t * password_key;
    uint8_t password_key_length;
    TLOGE("DeriveMasterKey\n");
    long rc = hwkey_open();
    TLOGE("DeriveMasterKey open rc:%d\n",rc);
    if (rc < 0) {
        TLOGE("DeriveMasterKey return\n");
        return rc;
    }
    ...
    hwkey_close(session);
    password_key = const_cast<uint8_t *>(master_key_.get());
    password_key_length = HMAC_SHA_256_KEY_SIZE;
    if(password_key != NULL)
        TLOGE("Master[%d]:%d  Master[0]:%d\n",password_key_length-1,password_key[password_key_length-1],password_key[0]);
    return rc;
}
...
bool TrustyGateKeeper::GetAuthTokenKey(const uint8_t **auth_token_key,
    ...
    *length = 0;
    *auth_token_key = NULL;
    TLOGE("GetAuthTokenKey\n");
    long rc = keymaster_open();
    TLOGE("keymaster_open rc:%d\n",rc);
    if (rc < 0) {
        TLOGE("keymaster_open return rc:%d\n",rc);
        return false;
    }
    ...
    TLOGE("keymaster_get_auth_token_key\n");
    rc = keymaster_get_auth_token_key(session, &key, &local_length);
    keymaster_close(session);
    TLOGE("keymaster_get_auth_token_key rc:%d \n",rc);

    if (rc == NO_ERROR) {
        *auth_token_key = key;
        *length = local_length;
        TLOGE("leave GetAuthTokenKey auth_token[%d]:%d\n",local_length-1,key[local_length-1]);
        return true;
    } else {
        TLOGE("keymaster_get_auth_token_key return rc:%d\n",rc);
        return false;
    }
}
...
void TrustyGateKeeper::ComputePasswordSignature(uint8_t *signature, size_t signature_length,
    ...
    TLOGE("salted[%d]:%d  salted[0]:%d\n",password_length + sizeof(salt)-1,salted_password[password_length + sizeof(salt)-1],salted_password[0]);
    ComputeSignature(signature, signature_length, key, key_length, salted_password,
            password_length + sizeof(salt));
    TLOGE("signature[%d]:%d  signature[0]:%d\n",signature_length-1,signature[signature_length-1],signature[0]);
}
...
uint64_t TrustyGateKeeper::GetMillisecondsSinceBoot() const {
    ...
    TLOGI("GetMillisecondsSinceBoot  time(ms):%lld \n",secure_time_ns / 1000 / 1000);
...
bool TrustyGateKeeper::WriteFailureRecord(uint32_t uid, failure_record_t *record, bool secure) {
    if (secure) {
//storge is not support now, use memory record
#if 0
